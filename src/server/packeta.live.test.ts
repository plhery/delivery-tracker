...
import { PacketaTracker, PacketaTrackingError } from './packeta';
...
describe('Packeta live anonymous tracking', () => {
  ...
  it('rejects the PacketaTrackingError contract', () => {
    expect(new PacketaTrackingError().status).toBe(404);
  });
});
