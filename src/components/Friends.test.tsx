...
describe('Friends', () => {
  ...
  it('previews privacy switches before saving and lets people leave the circle', async () => {
    ...
    const stats = within(sheet).getByRole('switch', { name: /^Passport stats/ });
    const arrival = within(sheet).getByRole('switch', { name: /^This week/ });
    ...
    expect(within(sheet).getByRole('button', { name: 'Save sharing preferences' })).toBeDisabled();
    await user.type(within(sheet).getByRole('textbox', { name: 'Nickname' }), '  Robin  ');
    await user.click(within(sheet).getByRole('button', { name: 'Save sharing preferences' }));
    await waitFor(() => expect(screen.queryByRole('dialog')).toBeNull());
    await user.click(screen.getByRole('button', { name: 'Sharing preferences' }));
    sheet = screen.getByRole('dialog');
    expect(within(sheet).getByRole('textbox', { name: 'Nickname' })).toHaveValue('Robin');
    expect(within(sheet).getByRole('switch', { name: /^Passport stats/ })).not.toBeChecked();
    ...
    await waitFor(() => expect(screen.queryByRole('dialog')).toBeNull());
    expect(screen.getByText('Good company. Small celebrations.')).toBeVisible();
    expect(screen.getByRole('button', { name: 'Create my Friends profile' })).toBeDisabled();
    await user.type(screen.getByRole('textbox', { name: 'Nickname' }), 'Robin');
    await user.click(screen.getByRole('button', { name: 'Create my Friends profile' }));
    expect(await screen.findByText('Room for your people')).toBeVisible();
  });
  it('makes demo invitations clearly fictional without calling a server', async () => {
    ...
    expect(screen.getByRole('dialog')).toHaveTextContent('Sign in to invite real friends');
    await user.click(within(screen.getByRole('dialog')).getByRole('button', { name: 'Close' }));
    await waitFor(() => expect(screen.queryByRole('dialog')).toBeNull());
    await user.click(screen.getByRole('button', { name: 'Have an invite code?' }));
    expect(screen.getByRole('dialog')).toHaveTextContent('Sign in to invite real friends');
  ...
  it('prevents duplicate mutations and ignores an in-flight action after backgrounding', async () => {
    ...
    const { user } = await show(client, false);
    await user.click(screen.getByRole('button', { name: 'Sharing preferences' }));
    const save = within(screen.getByRole('dialog')).getByRole('button', { name: 'Save sharing preferences' });
