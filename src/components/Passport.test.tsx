...
describe('Passport journal', () => {
  it('keeps stamps available without inventing timing or country records for a new passport', () => {
    ...
    const stamp = screen.getByRole('button', { name: 'Double digits, 0 / 10' });
    const details = document.getElementById(stamp.getAttribute('aria-controls')!);
    expect(details).not.toBeVisible();
    fireEvent.click(stamp);
    expect(details).toBeVisible();
    expect(details).toHaveTextContent('Ten delivered parcels');
    fireEvent.keyDown(stamp, { key: 'Escape' });
    expect(details).not.toBeVisible();
  });
