...
export function trackingLanguageStage(description: string): Stage | undefined {
  ...
  if (/will be delivered|sera livre|wird.*zugestellt|voraussichtlich.*zugestellt|sara consegnat/.test(text)) return 'registered';
  ...
  if (/clearance.*(?:has been completed|completed)|clearance (?:complete|success)|(?:leaving|departed from|left) customs|completion of customs|formalites.*(?:terminee|achevee)|released by (?:customs|a government agency)|customs (?:cleared|released)|\bcleared (?:through |by )?customs|dedouanement.*(?:termine|acheve)|fin du dedouanement|libere.*(?:douane|autorite)|douane.*libere|zollabfertigung.*abgeschlossen|(?:zoll|behorde).*freigegeben|sdoganamento.*completato|completamento.*sdoganamento|svincolat.*(?:dogana|autorita)/.test(text)) return 'in_transit';
