...
test('explains Amazon Logistics account tracking and blocks addition', async ({ page }, testInfo) => {
  const dialog = page.getByRole('dialog', { name: 'Add a parcel' });
  await dialog.getByLabel('Tracking number or link').fill('FR3000000001');
  await expect(dialog.getByText('Amazon Logistics', { exact: true })).toBeVisible();
  await expect(dialog.getByText(/Amazon Logistics deliveries are usually tracked in Your Orders on Amazon/)).toBeVisible();
