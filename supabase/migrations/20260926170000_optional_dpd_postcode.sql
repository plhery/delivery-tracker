-- Let DPD Switzerland parcels be added or switched to DPD without a delivery
-- postcode. DPD tracks without it and only uses it to unlock verified scans,
-- so a supplied postcode is still checked; GLS Switzerland still requires one.
-- The table constraint already allows a null postcode. Choosing the carrier
-- that recovery routing confirmed also replaces the tracking URL and postcode
-- saved with that route, so a cleared postcode is not reused later.
-- Everything else is copied from 20260921160000_add_ems.sql.
--
-- Apply this before deploying a server that accepts a missing DPD postcode.
...
begin
  ...
  if p_carrier is null or p_carrier not in (
    ...
    'yunexpress', 'zto', 'intl-post', 'unknown'
  ) then
