-- Keep distinct single-use invitations until accepted, expired, or revoked.
-- Existing tokens and read-only preview IDs remain valid through the migration.
alter table public.friend_invites drop constraint friend_invites_pkey;
alter table public.friend_invites add column id bigint generated always as identity primary key;
create index friend_invites_owner_order_idx on public.friend_invites(user_id, id);
  ...
  if p_action = 'save_profile' then
    ...
    if p_action = 'revoke_previous_invites' then
      select id into invite_id from public.friend_invites where user_id = actor
        and code_hash = encode(sha256(convert_to(p_code, 'UTF8')), 'hex') and expires_at > now();
      ...
      if p_code !~ '^[a-f0-9]{32}$' then raise exception 'Invalid invitation' using errcode = '22023'; end if;
