-- Extend the carrier catalog and recipient-postcode validation.
-- Preserve ownership, quotas, row locks and carrier-reset behavior.
  ...
  add constraint packages_carrier_check
    check (
      carrier in (
        ...
        'shipup', 'india-post', 'hermes-de', 'gls-de', 'delivengo',
        'intl-post', 'unknown'
      )
    ) not valid;
...
alter table public.packages
  ...
  add constraint packages_dpd_postcode_check
    check (
      ...
      or (carrier in ('mondial-relay', 'gls-de') and dpd_postcode ~ '^[0-9]{5}$')
...
begin
  ...
  if (p_carrier in ('dpd', 'gls-ch') and coalesce(normalized_postcode, '') !~ '^[0-9]{4}$')
      or (p_carrier in ('mondial-relay', 'gls-de') and coalesce(normalized_postcode, '') !~ '^[0-9]{5}$')
...
begin
  ...
  if (normalized_carrier in ('dpd', 'gls-ch') and coalesce(normalized_postcode, '') !~ '^[0-9]{4}$')
      or (normalized_carrier in ('mondial-relay', 'gls-de') and coalesce(normalized_postcode, '') !~ '^[0-9]{5}$')
