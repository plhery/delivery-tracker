-- An incident recovers on three healthy samples of its own kind and subject. When
-- a tier stops being probed (UPS and Swiss Post no longer record direct samples,
-- or a carrier simply has no parcel left), those samples never arrive and the
-- incident stayed active indefinitely. It now closes once every sample that
-- opened it has aged out of the 24-hour window. No recovery event is sent, because
-- no recovery was observed; a tier that is still failing when traffic resumes
-- opens a new incident as before.
...
begin
  ...
  -- Recovery needs three healthy samples of the same kind and subject. A tier that is
  -- no longer probed never produces them, so its incident would stay open for ever.
  -- Close it once its evidence has expired, without a recovery notice: nothing was observed.
  update public.tracking_health_incidents i set active=false
    where i.active and i.pending is null and not exists(
      select 1 from public.tracking_health_samples s where s.kind=i.kind and s.subject=i.subject);
