...
create function public.auto_link_package_tracking(p_user_id uuid default null)
returns integer
language plpgsql
set search_path = pg_catalog
as $$
  ...
  for pair in
    with matches as (
      ...
      join public.packages delivery on delivery.user_id = original.user_id
        ...
        and delivery.carrier_data->>'international_tracking_number'
          = coalesce(nullif(original.carrier_data->>'canonical_tracking_number', ''), original.tracking_number)
      where original.carrier in ('gls-de', 'gls-ch', 'gls-fr')
        and original.archived_at is null
        and not (original.carrier_data ? 'original_carrier')
  ...
  loop
    ...
    if exists (
      ...
      where original.id = pair.original_id and delivery.id = pair.delivery_id
        and original.carrier in ('gls-de', 'gls-ch', 'gls-fr') and delivery.carrier = 'swiss-post'
        and original.archived_at is null and delivery.archived_at is null
        and not (original.carrier_data ? 'original_carrier') and not (delivery.carrier_data ? 'original_carrier')
        and delivery.current_stage not in ('pending', 'registered')
        and delivery.carrier_data->>'international_tracking_number'
          = coalesce(nullif(original.carrier_data->>'canonical_tracking_number', ''), original.tracking_number)
