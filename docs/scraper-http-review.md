...
| DHL / DHL eCommerce | DHL Paket keeps direct structured tracking first with TRAWL recovery. DHL eCommerce's `utapi` answers every direct server request with an Akamai crypto proof-of-work challenge (HTTP 428); cookie replay and page-visit-first both still return 428, so no HTTP-only path exists and the adapter goes straight to local Chromium, where the site solves the challenge and retries the exact API URL. | DHL eCommerce: straight-to-browser (direct dropped 2026-09-10); DHL Paket keeps the verified recovery. |
