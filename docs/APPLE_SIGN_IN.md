...
2. Create a web Services ID, for example `com.plhery.SwissDeliveryTracker.web`, associated with that app. Register your public Supabase Auth hostname (for example `supabase.example.com`) as its domain and `https://supabase.example.com/auth/v1/callback` as its return URL. Substitute your own hostname in both Apple and Supabase configuration.
