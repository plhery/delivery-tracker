...
- `component:tracking-routing operation:carrier_coverage_discovered` — a provider named a carrier the catalog does not know at all: a candidate catalog entry or name alias.
...
Existing sync attempt/step audits remain in place. Cross-provider failures are reported before recovery, so a successful fallback does not conceal them. Internal tier warning timing is documented in [scraper monitoring](scraper-monitoring.md). Shadow checks and provider cooldowns limit issue volume. Carrier names a universal provider reports are hints, not proven new adapters, and stay in the parcel's routing state (`reported_carriers_seen`). A name maps to a carrier when it is the catalog's name or alias, or that name followed by the carrier's own single-zone country ("Chronopost France"). Only a name the catalog does not know at all is reported, once per parcel: a catalog carrier named as one leg of a handoff, with any country, or another network of DHL, DPD, GLS or Hermes ("DHL Express", "GLS Italy") is no discovery. The reporter uses the existing `SENTRY_DSN`; this change does not create organization-level alert recipients or notification rules.
